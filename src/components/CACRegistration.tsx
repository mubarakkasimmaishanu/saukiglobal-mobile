...
import { ChevronLeft, Briefcase, ShieldCheck, CheckCircle2, ArrowRight, FileText } from 'lucide-react';
import { useUser } from '../context/UserContext';
import { api } from '../services/api';
import PinInput from './PinInput';

interface CACRegistrationProps {
  onBack: () => void;
}

export default function CACRegistration({ onBack }: CACRegistrationProps) {
    ...
    <div className="min-h-screen bg-[#111415] text-[#e1e3e4] font-sans mesh-gradient">
      <div className="max-w-md mx-auto relative px-6 pb-12">
        <header className="py-8 flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold tracking-tight">CAC Registration</h1>
        </header>

        {step === 'form' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="glass-panel p-6 mb-8 bg-amber-500/5 border-amber-500/20">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-amber-600 text-white flex items-center justify-center">
                  <Briefcase size={24} />
                </div>
                <div>
                  <h2 className="text-sm font-black text-white uppercase tracking-wider">Business Name</h2>
                  <p className="text-[10px] text-amber-400 font-bold">Official Registration</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); setStep('pin'); }} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#66df75] uppercase tracking-widest px-1">Proposed Business Name</label>
                <input
                  type="text"
                  value={businessName}
                  ...
                  placeholder="e.g. SaukiGlobal Tech"
                  required
                />
              </div>
              <button type="submit" disabled={!businessName} className="w-full btn-primary py-5 flex justify-center items-center gap-3 disabled:opacity-50 transition-all uppercase tracking-widest font-black text-sm">
                Proceed (₦{price.toLocaleString()}) <ArrowRight size={20} />
              </button>
            </form>
          </div>
        )}
          ...
          <div className="animate-in slide-in-from-bottom-8 duration-500 pt-8">
            <div className="text-center mb-10">
              ...
              <p className="text-xs text-[#e1e3e4]/40 font-medium px-8 leading-relaxed">
                Registering <span className="text-white font-bold">{businessName}</span> for <span className="text-[#66df75] font-black">₦{price.toLocaleString()}</span>.
              </p>
            </div>
        ...
        {step === 'success' && (
          <div className="animate-in zoom-in-95 duration-500 pt-8">
            <div className="glass-panel p-8 text-center border-[#66df75]/20">
              ...
              <p className="text-[10px] text-[#66df75] font-black uppercase tracking-[0.3em] mb-8">Admin is processing your request</p>
